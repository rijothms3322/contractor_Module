...
import AddReminderTour from '@/components/tours/AddReminderTour';
import DashboardTour from '@/components/tours/DashboardTour';
import FamilySyncTour from '@/components/tours/FamilySyncTour';
...
export const DashboardView: React.FC = () => {
  ...
  const [showAddReminderTour, setShowAddReminderTour] = useState(false);
  const [showFamilySyncTour, setShowFamilySyncTour] = useState(false);
  ...
  const [frequency, setFrequency] = useState<
    "every_day" | "specific_days" | "interval"
  >("every_day");
  ...
  useEffect(() => {
    if (isAddReminderOpen) {
      if (!user?.isMedicineWalkthroughShown) {
        const timer = setTimeout(() => setShowAddReminderTour(true), 600);
        return () => clearTimeout(timer);
      }
    }
  }, [isAddReminderOpen]);
  ...
  useEffect(() => {
    if (showSyncModal && !activeFamily) {
      if (!user?.isPrescriptionWalkthroughShown) {
        const timer = setTimeout(() => setShowFamilySyncTour(true), 400);
        return () => clearTimeout(timer);
      }
    } else {
      setShowFamilySyncTour(false);
    }
  }, [showSyncModal, activeFamily]);
  ...
  const handleReminderSubmit = async (e: React.FormEvent) => {
    ...
    // --- Request Web Notification Permission (only on web) ---
    ...
    if (frequency !== "interval" && newSelectedTimings.length === 0) {
      setValidationError("Please select at least one timing slot (Morning, Afternoon, Evening, Night).");
      return;
    }
  ...
  const handleEditClick = (medicineId: string, familyMemberId: string | null) => {
    ...
    console.log('Editing medicine ID:', medicineId, 'Found med:', med);
    if (!med) return;
    ...
    return (
      <div key={r.id} className={`glass-card p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center justify-between border-l-4 transition-all gap-3 ${borderClass}`}>
        <div className="flex items-center gap-4 min-w-0">
          <div className="w-11 h-11 rounded-xl bg-surface-container-highest flex items-center justify-center shrink-0 overflow-hidden border border-outline-variant/10">
            {r.recipientAvatar ? (
        ...
        <div className="flex items-center gap-2 justify-end self-end sm:self-auto">
          {r.status === "pending" && (
            <>
              <button
                type="button"
                onClick={() => handleEditClick(r.medicineId, r.familyMemberId || '')}
  ...
  return (
    <>
      ...
      {isAddReminderOpen && showAddReminderTour && (
        <AddReminderTour />
      )}
      <div className="space-y-stack-lg pb-16">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white/40 backdrop-blur-xl border border-white/40 p-6 shadow-sm flex items-center justify-between gap-6 transition-all duration-300 hover:shadow-md hover:translate-y-[-1px] group">
          ...
          <div className="relative z-10 w-24 h-16 sm:w-32 sm:h-20 shrink-0 flex items-center justify-center opacity-85 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <div className="flex justify-between items-center glass-card bg-linear-to-br from-secondary-container/20 to-white p-5 rounded-2xl border border-outline-variant/20 shadow-sm">
        ...
        {todayReminders.length === 0 ? (
          <div className="glass-card rounded-3xl p-8 text-center border shadow-md border-outline-variant/20 flex flex-col items-center justify-center gap-3">
            <span className="material-symbols-outlined text-5xl text-primary">Event_busy</span>
            <h3 className="font-headline-md text-lg text-secondary font-bold">No Scheduled Doses</h3>
            <p className="font-body-md text-sm text-on-surface-variant max-w-sm">
              You have no medicine reminders scheduled for today. Tap the “Add Reminder” button above to create your first one.
            </p>
          </div>
        ) : (
            ...
            <section className="relative space-y-3">
              <div className="flex items-center gap-2.5">
                ...
                {morningReminders.length > 0 && (
                  <span className={`font-label-sm text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isMorningDone ? "bg-tertiary/10 text-tertiary" : "bg-primary/10 text-primary"
                    }`}>
              ...
              <div className="space-y-3">
                {morningReminders.length === 0 ? (
                  <div className="glass-card p-4 rounded-2xl shadow-sm opacity-60 flex gap-4 items-center">
                    <div className="w-10 h-10 rounded-full bg-surface-container-low flex items-center justify-center text-on-surface-variant/40 shrink-0">
            ...
            <section className="relative space-y-3">
              <div className="flex items-center gap-2.5">
                ...
                {afternoonReminders.length > 0 && (
                  <span className={`font-label-sm text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isAfternoonDone ? "bg-tertiary/10 text-tertiary" : "bg-primary/10 text-primary"
                    }`}>
            ...
            <section className="relative space-y-3">
              <div className="flex items-center gap-2.5">
                ...
                {eveningReminders.length > 0 && (
                  <span className={`font-label-sm text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isEveningDone ? "bg-tertiary/10 text-tertiary" : "bg-primary/10 text-primary"
                    }`}>
            ...
            <section className="relative space-y-3">
              <div className="flex items-center gap-2.5">
                ...
                {nightReminders.length > 0 && (
                  <span className={`font-label-sm text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isNightDone ? "bg-tertiary/10 text-tertiary" : "bg-primary/10 text-primary"
                    }`}>
        ...
        <section
          ...
          className="glass-card rounded-3xl p-6 sm:p-7 shadow-md border border-outline-variant/20 bg-linear-to-br from-orange-50/30 to-white flex items-center justify-between gap-5 cursor-pointer hover:border-primary/40 hover:shadow-lg transition-all duration-200 group active:scale-[0.99] text-left mt-4"
        ...
        >
          <div className="flex gap-5 items-center min-w-0">
            {(() => {
              ...
              return (
                <>
                  <div className="w-14 h-14 rounded-2xl bg-orange-50 border border-orange-100 flex items-center justify-center shrink-0 shadow-sm group-hover:scale-105 transition-transform duration-200">
        ...
        <section id="family-sync-card" className="glass-card rounded-2xl p-5 shadow-sm border border-outline-variant/20 bg-linear-to-br from-primary-container/10 to-white flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex gap-4 items-start text-left">
            <div className="w-10 h-10 rounded-xl bg-primary-container text-white flex items-center justify-center shrink-0 shadow-sm animate-pulse">
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 bg-secondary hover:bg-opacity-95 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 active:scale-95 transition-all shadow-sm shrink-0"
          >
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
          <div className="glass-card p-6 rounded-2xl shadow-sm bg-linear-to-br from-secondary-container/20 to-white border border-outline-variant/20 flex flex-col justify-between">
            <div>
                  ...
                  {past7DaysData.map((d, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                        <div
                          className={`w-full rounded-t-md transition-all duration-300 ${d.value === -1
          ...
          <div id="refill-alert-card" className="glass-card p-6 rounded-2xl shadow-sm border border-outline-variant/20 flex flex-col justify-between min-h-36">
            <div>
              ...
              <div className="space-y-4 text-left">
                ...
                <OCRUploader
                  onComplete={(data) => {
                    ...
                    // OCR Text
                    console.log(data.text, 'OCRUploader');

                    // Medicines
                    console.log(data.medicines, 'OCRUploader');

                    // Report
                    console.log(data.report, 'OCRUploader');
                  }}
                  onError={(err) => {
                    console.error(err);
                  }}
          ...
          <div className="fixed inset-0 bg-inverse-surface/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-[420px] bg-white rounded-3xl p-6 shadow-2xl border border-outline-variant/30 flex flex-col max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handleReminderSubmit} className="space-y-4 text-left">
                ...
                <div id="add-private-checkbox" className="space-y-1.5 p-3 rounded-2xl bg-orange-50/30 border border-orange-200/40 flex items-center justify-between">
                  <div>
                ...
                <div id="add-duration" className="space-y-2 bg-surface-container-low/40 p-3 rounded-2xl border border-outline-variant/10">
                  ...
                  <div className="flex flex-col justify-center gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFrequency(
                            e.target.value as "every_day" | "specific_days" | "interval"
                          )
                        }
                    ...
                    {frequency === 'interval' && (
                      <div className='flex flex-col gap-2'>
                        <span className='text-sm'>
                          Remind every
                          <button
                            className='border border-primary px-2 py-1 rounded ml-3'
                            onClick={() => {
                              // You can also add a dropdown for hours if needed
                              // For simplicity, we just toggle a number picker or keep as is
                              // But we can add a simple increment/decrement or just keep static for now
                              // If you want a dropdown for hours, implement similarly
                            }}
                          >
                            {intervalHours} Hours
                          </button>
                        ...
                        <span className='text-sm relative'>
                          ...
                          >
                            {allTimeOptions.find(o => o.value === intervalStartTime)?.label || '8:00 AM'}
                          </button>
                          {showStartTimeDropdown && (
                            <div className="absolute mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto z-50 w-40">
                              {allTimeOptions.map(opt => (
                                <div
                                  key={opt.value}
                                  className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm"
                                  onClick={() => {
                                    setIntervalStartTime(opt.value);
                    ...
                    {frequency === "specific_days" && (
                      <div className="inline-flex w-full p-1 rounded-lg bg-orange-50/30 border border-orange-200/60">
                        <button
                          ...
                          className={`flex-1 px-4 py-2=1 text-sm font-medium rounded-md transition-all duration-200 ${selectedDayType === "weekdays"
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setShowWeekOptions(false);
                            setSelectedDayType("normal")
                          }}
                          className={`flex-1 px-4 py-1 text-sm font-medium rounded-md transition-all duration-200 ${selectedDayType === "normal"
                            ? "bg-white text-orange-600 shadow-sm ring-1 ring-orange-200"
                            : "text-gray-500 hover:text-orange-600 hover:bg-orange-100/50"
                            }`}
                        >
                          ...
                          <div className="max-h-52 overflow-y-auto">
                            {[1, 2, 3, 4, 5, 7, 14, 30, 60, 90, 100].map((days) => (
                              <button
                                key={days}
                                type="button"
                                onClick={() => setRepeatEveryNDays(days)}
                ...
                {/* Timing Slots and Preferred Intake Times */}
                {frequency !== "interval" &&
                  <>
                    <div id="add-timings" className="space-y-1.5">
                    ...
                    <div id="add-intake-time-select" className="space-y-1.5 bg-surface-container-low/40 p-3 rounded-2xl border border-outline-variant/10">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full border text-xs font-bold transition-all ${newFamilyMemberId === "" && !isAddingNewPerson
                    ...
                    {familyMembers.map((fm) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full border text-xs font-bold transition-all ${newFamilyMemberId === fm.id && !isAddingNewPerson
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full border border-dashed text-xs font-bold transition-all flex items-center gap-1 ${isAddingNewPerson
                      ...
                      <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-thin">
                        {AVATAR_CATEGORIES.map((cat) => (
                          <button
                            ...
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-colors shrink-0 ${activeAvatarCategory === cat.id
                        ...
                        {AVATAR_ITEMS.filter((av) => av.category === activeAvatarCategory).map((av) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-11 h-11 rounded-full p-0.5 border-2 transition-all flex items-center justify-center overflow-hidden shrink-0 ${isSelected ? "border-primary scale-110 shadow-sm" : "border-transparent hover:scale-105"
                                }`}
                              title={av.label}
                            >
                      ...
                      {[5, 10, 15, 30, 60].map((mins) => {
                        ...
                        return (
                          <button
                            ...
                            className={`py-2 text-center rounded-xl font-bold text-xs transition-all ${isSelected && customSnoozeHours === 0 && customSnoozeMins === 0
